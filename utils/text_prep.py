...
def clean(tweet):
    ...
    if type(tweet) == str:
        ...
        return ' '.join([word for word in clean_p if word.lower() not in stopwords.words('english') +['im', 'day','get','go', 'dont', 'today', 'going', 'got', 'work', 'one', 'cant', 'time', 'know', 'back', 'really', 'see', 'mothers', 'want', 'home', 'night', 'still', 'new', 'think', 'much', 'well', 'thanks', 'last', 'morning', 'need', 'tomorrow', 'this', "that"] ])
    else:
        return ' '
